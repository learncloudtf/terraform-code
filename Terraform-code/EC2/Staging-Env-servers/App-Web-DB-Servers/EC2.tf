...
#1.Provisioning 4x r5a.2xlarge RHEL Linux machine with 100GB EBS (OS: 100GB) app_subent
resource "aws_instance" "Machine-ra5-2xlarge-01" {
  ...
  tags = {
    Name = "${format("nets_oa_vpc_2-app-Stage-rtgs-backoffice-1-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}
...
resource "aws_instance" "Machine-ra5-2xlarge-02" {
  ...
  tags = {
    #Name = "nets_oa_vpc_2-EC2-RDBMS"
    Name = "${format("nets_oa_vpc_2-Stage-RDBMS-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}

#3.Provisioning 2x r5a.xlarge RHEL Linux machine with 100GB EBS (OS: 100GB) app_subent
resource "aws_instance" "Machine-r5a-xlarge" {
  count = 2   
  ...
  tags = {
    Name = "${format("nets_oa_vpc_2-app-Stage-sss-batch-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}
...
resource "aws_instance" "Machine-r5a-large" {
  ...
  tags = {
    Name = "${format("nets_oa_vpc_2-app-Stage-sss-report-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}
...
resource "aws_instance" "Machine-t3a-2xlarge" {
  ...
  tags = {
   #Name = "nets_oa_vpc_2-app-EC2-Sanction-Screening"
    Name = "${format("nets_oa_vpc_2-app-Stage-Sanction-Screening-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}

#6.Provisioning 6x t3a.xlarge RHEL Linux machine with 104GB EBS (OS: 100GB) app_sunet
resource "aws_instance" "Machine-t3a-xlarge" {
  count = 6   
  
  ami = var.ami
  instance_type = var.instance_type["t3a_xlarge"]
  subnet_id = "${element(var.subnet_ids_app, count.index)}"
  key_name      = var.key_name
  # root disk
  root_block_device {
  ...
  tags = {
    Name = "${format("nets_oa_vpc_2-app-Stage-rtgs-qp-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}
...
resource "aws_instance" "Machine-t3a-large" {
  ...
  tags = {
    Name = "${format("nets_oa_vpc_2-Stage-sss-web-portal-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}

#8.Provisioning 14x t3a.medium RHEL Linux machine with 104GB EBS (OS: 100GB) web_subnet
resource "aws_instance" "Machine-t3a-medium-01" {
  ...
  tags = {
    Name = "${format("nets_oa_vpc_2-Stage-rtgs-web-portal-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}
...
resource "aws_instance" "Machine-t3a-medium-02" {
  ...
  tags = {
    #Name = "nets_oa_vpc_2-app-EC2-Solace-PubSub"
    Name = "${format("nets_oa_vpc_2-app-Stage-Solace-PubSub-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
}
...
resource "aws_instance" "Machine-t2-small" {
  ...
  tags = {
    #Name = "nets_oa_vpc_2-app-EC2-Solace-Monitoring"
    Name = "${format("nets_oa_vpc_2-app-Stage-Solace-Monitoring-%03d", count.index + 1)}"
    Environment = "Stage"
    Backup = "Yes"
   }
} 
